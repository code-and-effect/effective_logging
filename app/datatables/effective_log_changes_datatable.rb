class EffectiveLogChangesDatatable < Effective::Datatable
  ...
  collection do
    base = Effective::Log.logged_changes.deep
    logged_parent = base.where(parent_id: attributes[:log_id])
    logged_changes = base.where(changes_to_type: attributes[:changes_to_type], changes_to_id: attributes[:changes_to_id])

    logged_parent.or(logged_changes)
  end
